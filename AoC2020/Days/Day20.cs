		...
		static void Day20(List<string> input)
		{
			...
			while (line < input.Count)
			{
				if (input[line].StartsWith("Tile"))
				{
					...
					{
						if ((i & 1) == 1)
							panel[i] = HelpF.RotateMatrix(panel[i]);
						if ((i & 2) == 2)
							panel[i] = FlipVert(panel[i]);
						if ((i & 4) == 4)
							panel[i] = FlipHori(panel[i]);
					}
					//Console.WriteLine(panelNum);
					panels.Add(panel);
				}
				line++;
			}

			/*foreach (var card in panels)
			{
				Console.WriteLine(card[0][0, 0].id);
				for (int k = 0; k < 8; k++)
				{
					Console.WriteLine(k);
					for (int i = 0; i < CardSize; i++)
				...
				for (int i = miny; i <= maxy; i++)
				{
					for (int j = minx; j <= maxx; j++)
					{
						foreach (var item in panels)
						{
							for (int k = 0; k < 8; k++)
							{
								if (PlaceCard(j, i, item[k])) goto panelPicked;
							}
						}
					panelPicked:;
					}
			...
			/*for (int i = miny; i <= maxy; i++)
			{
				for (int j = minx; j <= maxx; j++)
				{
					Console.Write(image.TryGetValue((j, i), out var c) ? c.sym : ' ');//].sym);
					if (Mod(j, CardSize) == CardSize - 1) Console.Write(' ');
				}
				Console.WriteLine();
				if (Mod(i, CardSize) == CardSize-1) Console.WriteLine();
			}*/
			
			Console.WriteLine(image[(minx, miny)].id * image[(maxx, miny)].id * image[(minx, maxy)].id * image[(maxx, maxy)].id);
			...
			for (int i = 1; i < 8; i++)
			{
				map[i] = map[0];
				if ((i & 1) == 1)
					map[i] = HelpF.RotateMatrix(map[i]);
				if ((i & 2) == 2)
			...
			Console.WriteLine(roughness.Min());

			bool PlaceCard(int x, int y, (char sym, long id)[,] card, bool skipPlaceCheck = false)
			{
